...
class GeMScraper(BaseScraper):
    ...
    def __init__(self):
        ...
        if not self.api_key:
            logger.warning(
                "[GeM] No API key found! "
                "Add DATAGOV_API_KEY to .env file. "
    ...
    def fetch_gem_stats(self) -> dict:
        ...
        if html and len(html) > 500:
            ...
            return {
                ...
                "note":       "HTML page fetched - using sample stats structure",
                "stats":      self._get_sample_stats()["stats"],
            }

        logger.warning("[GeM] Could not fetch stats page, using sample")
    ...
    def _get_sample_contracts(self) -> list:
        ...
        Shows the type of data we will eventually link to politicians.
        """
        return [
            {
            ...
            },
            {
                ...
                "scraped_at":   datetime.now().isoformat(),
                "entity_type":  "contract",
                "note":         "SAMPLE - same company won 2nd contract (pattern!)",
            },
        ]
