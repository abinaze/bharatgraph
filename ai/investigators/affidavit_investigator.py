...
# BUG-C1 FIX: file used module-level functions only -- no class.
# __init__.py: from affidavit_investigator import AffidavitInvestigator
# raised ImportError, crashing ALL_INVESTIGATORS and the /investigate endpoint.
# BUG-H1 FIX: do NOT fabricate synthetic history (old code multiplied assets by 0.3).
class AffidavitInvestigator:
    NAME   = NAME
    FOCUS  = FOCUS
    WEIGHT = WEIGHT
    ...
    def investigate(self, entity_id: str, entity_name: str,
        ...
        if session:
            try:
                rows = session.run(
                    "MATCH (p:Politician {id:})-[:FILED_AFFIDAVIT]->(a:Affidavit) "
                ...
                history = [
                    {"year": r["year"],
                     "total_assets_crore": r.get("total", 0) or 0,
                     "movable_assets_crore": r.get("movable", 0) or 0}
                    for r in rows if r.get("year")
                ]
            except Exception as e:
        ...
        if len(history) >= 2:
            ...
            positive.append(
                "Insufficient affidavit history for trajectory analysis "
                "(fewer than 2 election cycles in current dataset)."
            )
