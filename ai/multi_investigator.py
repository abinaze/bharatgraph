...
def run_investigator(investigator_class, entity_id: str,
    ...
    try:
        if driver:
            ...
            # indication that the database was offline.
            logger.warning(
                f"[MultiInvestigator] {investigator_class.NAME}: "
...
def synthesise(results: list) -> dict:
    ...
    for result in results:
        for finding in result.get("findings", []):
            ...
            finding_counter[ftype]["investigators"].append(result["investigator"])

    # BUG-8 FIX: filter was `>= 3` which silently dropped all MODERATE findings
    # (those agreed by exactly 2 investigators).  Changed to `>= 2` so MODERATE
    # findings are included in the agreed_findings output.
    agreed = [f for f in finding_counter.values() if f["count"] >= 2]
    for f in agreed:
        if f["count"] >= 3:
            f["confidence"] = "HIGH"
        elif f["count"] == 2:
            f["confidence"] = "MODERATE"   # now reachable
        else:
            f["confidence"] = "LOW"
...
class MultiInvestigator:
    ...
    def investigate(self, entity_id: str, entity_name: str = "") -> dict:
        ...
        logger.info(f"[MultiInvestigator] Starting: {entity_name or entity_id}")
        logger.info(f"[MultiInvestigator] Report hash: {report_hash[:16]}...")
        ...
        agreed_count = len(synthesis["agreed_findings"])
        explanation_raw = (
            f"The multi-investigator analysis of {entity_name} produced "
            f"{agreed_count} agreed finding(s) confirmed by 2 or more investigators. "
        ...
        logger.success(
            ...
            f"Agreed findings: {agreed_count}"
        )
        return report


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="BharatGraph Multi-Investigator Engine")
    ...
    print(f"  Report hash:       {report['unique_report_hash'][:32]}...")
