...
from api.routes import search, profile, graph, risk, multilingual, export, admin, investigation, affidavit, biography, benami, sources, procurement, conflict, linguistic, policy, adversarial
    ...
    try:
        if driver:
            with driver.session() as s:
                row = s.run("MATCH (m:PipelineMeta) RETURN m.last_run AS ts ORDER BY m.last_run DESC LIMIT 1").single()
...
def debug_neo4j():
    ...
    try:
        ...
        return {"status": "failed", "error": "Connection failed — check environment secrets", "uri_prefix": uri[:20] if uri else "NOT SET"}
